...
public class LabelPropagationExample implements ProgramDescription {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStream<LabelPropagationGlobalInfo> lp = edges.aggregate(new LabelPropagation(mergeWindowTime));
        lp.print();
        env.execute("Label Propagation");
    }
    ...
    private static boolean parseParameters(String[] args) {
        ...
        if (args.length > 0) {
            if (args.length != 3) {
                System.err.println("Usage: LabelPropagationExample <input edges path> <merge window time (ms)> "
            ...
            System.out.println("Executing LabelPropagationExample example with default parameters and built-in default data.");
            System.out.println("  Provide parameters to read input data from files.");
            System.out.println("  See the documentation for the correct format of input files.");
            System.out.println("  Usage: LabelPropagationExample <input edges path> <merge window time (ms)> "
                    + "print window time (ms)");
        }
        return true;
    }
...
//                new Edge<>(5l, 6l, 1.0));
    ...
    public String getDescription() {
        return "Streaming Label Propagation";
    }
}